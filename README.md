...
이는 매장 운영자가 여러 시스템을 번갈아 사용할 필요 없이 하나의 애플리케이션으로 모든 업무를 해결할 수 있으며 무엇보다 회원 아이디로 가볍고 직관적이며, 군더더기없는 서비스와 확장성을 제공하기에 경쟁 서비스와 차별화됩니다.
<br><br>
...
### 상세 매출 리포트: 
일별 및 월별 매출내역을 제공하며, 날짜 범위 필터링과 캘린더 뷰를 통해 직관적인 데이터 탐색이 가능합니다. <br>
...
회고 : 캐싱을 사용하면 프론트엔드 선에서 간단히 마무리 될거라 생각했지만, 이후에 다른 스토어에서 다른 카테고리를 불러와야 하는 등
의 과정에서 캐시가 중첩되는 현상을 해결하느라 불필요한 수정이 많이 필요했습니다. 간단한 응답값은 기초 회의에서 미리 마련해서 백엔드에
요청해야겠다는 생각을 할 수 있었던 경험이었습니다.

PR Comment #1 [Link](https://github.com/store-manager2025/store.app/pull/11)

      
### 2. React 상태 업데이트 지연 문제<br>
...
회고 : 비동기 문제를 피하려면 상태 관리 로직을 더 명시적으로 설계하거나, 필요하면 외부 상태 관리 라이브러리를 적극 활용해야겠다 싶었고 
앞으로는 상태 업데이트와 데이터 전송 시점이 엇갈릴 가능성을 미리 점검하고, 디버깅 과정에서 콘솔 로그나 상태 변화 추적을 더 꼼꼼히 해봐야겠다는 생각이 들었습니다.

### 3. 스토어 전환 시 남아있는 캐시와 잘못된 카테고리 로드 문제

문제: SettingPage에서 "Transfer"로 스토어를 전환할 때 이전 스토어의 카테고리와 메뉴가 로드되는 문제가 발생.
원인: usePosStore의 menuCache는 storeId별로 분리되었으나, selectedCategoryId와 currentMenus가 초기화되지 않아 이전 상태가 계속 랜더링됬습니다.
해결: transfer시 캐시를 초기화 시키고, setStoreId에서 상태를 초기화하고, PosPage의 useEffect를 개선해 새 카테고리를 강제로 로딩하도록 분명히 했습니다.
결과: 새로운 스토어의 카테고리와 메뉴가 정확히 로드됨.

회고: 캐시만 수정하면 될 줄 알았으나, 상태 초기화 누락이 문제였습니다. 디버깅 시에 트리거가 되는 문제를 먼저 찾아서 로그를 달며
상태 변화를 추적했다면 좀더 빨리 문제를 해결 했을 수 있었을텐데 엄한데서 여러번 수정을 거치다보니 시간이 많이 소요되었습니다. 근원을 찾도록 하는 
습관을 길러야 겠다는 것을 배웠습니다.
